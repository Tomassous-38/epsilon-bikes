...
### 2026-03-01 — Replace Gotham with Montserrat

**Status:** Accepted
**Context:** Gotham font was loaded via restricted cross-origin @font-face URLs (flagged with @FONTWARNING). Needed a freely available Google Font replacement.
**Decision:** Use Montserrat (Google Fonts) as the drop-in replacement. Kept the Tailwind key name `gotham` so no component files needed updating.
**Alternatives Considered:**
- Nunito Sans — similar feel but slightly more rounded/casual than Gotham's geometric precision.
- DM Sans — clean but narrower x-height; less faithful to Gotham's proportions.
**Consequences:** Zero component changes required; Montserrat loads reliably via Google CDN with matching weights (400/500/700).
